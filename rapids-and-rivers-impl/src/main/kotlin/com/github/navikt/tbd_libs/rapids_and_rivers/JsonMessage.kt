...
open class JsonMessage(
    ...
    internal val participatingServices: List<String>?
        get() = json.path(ParticipatingServicesKey)
            .takeIf { it.isArray && it.size() > 0 }
            ?.mapNotNull { jsonNode -> jsonNode.path("service").takeUnless { it.isMissingOrNull() }?.asString() }

    internal val behov: List<String>?
        get() = json.path(NeedKey)
            .takeIf { it.isArray && it.size() > 0 }
            ?.mapTo(mutableListOf()) { it.asString() }
    ...
    private val tracing =
        mutableMapOf<String, Any>(
            ...
            compute("behov") { _, _ -> json.path(NeedKey).mapTo(mutableListOf(), JsonNode::asString).takeUnless(List<*>::isEmpty) }
        }.toMap()
...
Se kodeeksempel https://github.com/navikt/tbd-libs/blob/main/rapids-and-rivers/README.md""",
    ...
    fun demandAll(key: String, values: List<String>) {
        val node = node(key)
        if (node.isMissingNode) problems.severe("Missing demanded key $key")
        if (!node.isArray || !node.mapTo(mutableListOf(), JsonNode::asString).containsAll(values)) problems.severe("Demanded $key does not contains $values")
        accessor(key)
    }
...
Se kodeeksempel https://github.com/navikt/tbd-libs/blob/main/rapids-and-rivers/README.md""",
    ...
    fun demandAllOrAny(key: String, values: List<String>) {
        val node = node(key)
        if (node.isMissingNode) problems.severe("Missing demanded key $key")
        if (!node.isArray || node.mapTo(mutableListOf(), JsonNode::asString).none { it in values }) problems.severe("Demanded array $key does not contain one of $values")
        accessor(key)
    }
...
Se kodeeksempel https://github.com/navikt/tbd-libs/blob/main/rapids-and-rivers/README.md""",
    ...
    fun requireAllOrAny(key: String, values: List<String>) {
        val node = node(key)
        if (node.isMissingNode) return problems.error("Missing required key $key")
        if (!node.isArray || node.mapTo(mutableListOf(), JsonNode::asString).none { it in values }) {
    ...
    fun requireAll(key: String, values: List<String>) {
        val node = node(key)
        if (node.isMissingNode) return problems.error("Missing required key $key")
        if (!node.isArray || !node.mapTo(mutableListOf(), JsonNode::asString).containsAll(values)) {
